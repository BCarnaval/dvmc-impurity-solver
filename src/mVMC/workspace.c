...
void initializeWorkSpaceAll() {
  ...
  //printf("Workspace info 1: %p\n", WorkSpaceThreadDouble[0]);
...
void RequestWorkSpaceThreadDouble(int n) {
  ...
  if(n>NumWorkSpaceThreadDouble) {
    ...
    #pragma omp parallel private(i)
    {
      ...
      //WorkSpaceThreadDouble[i] = (double*)calloc(NumWorkSpaceThreadDouble,sizeof(double));
      WorkSpaceThreadDoubleNow[i] = WorkSpaceThreadDouble[i];
      //printf("Workspace info: %p %p\n", WorkSpaceThreadDouble[i], WorkSpaceThreadDoubleNow[i]);
    }
  }
  return;
}
