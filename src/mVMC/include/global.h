...
#define D_FileNameMaxL 256

/***** definition *****/
char CDataFileHead[D_FileNameMax]; /* prefix of output files */
char CParaFileHead[D_FileNameMax]; /* prefix for optimized variational
                                      parameters */
...
int PrintEnergy; /* 0: do not print individual energy measurements
                    1: print individual energy measurements*/

int PrintConfig; /* 0: do not print individual configurations
                    1: print individual configurations*/

int PrintProgress; /* 0: do not print progress bar
                    1: print progress bar*/

int ExactSample; /* 0: generate configurations via MC
                    1: genereate full set of configurations exactly */
...
int NMPTrans;    /* the number of quantum projection for translation and point
                    group symmetry */
int NQPFull;     /* the total number of quantum projection =
                    NSPGaussLeg*NMPTrans*NQPTransOpt */
int NQPFix;      /* for QPFixWeight NSPGaussLeg*NMPTranss */

int NSROptItrStep; /* the number of SR method steps */
int NSROptItrSmp;  /* the number of SR method steps for calculation of average
                      value */
int NSROptFixSmp;  /* the number of SR method steps with fixed samples (1 is
                      recommended) */

double DSROptRedCut; /* SR stabilizing factor for truncation of redundant
                        directions */
double
    DSROptStaDel; /* SR stabiliaing factor for diagonal element modification */
...
int NDoublonHolon2siteIdx,
    **DoublonHolon2siteIdx; /* DoublonHolon2siteIdx[idx][2*Nsite] */
int NDoublonHolon4siteIdx,
    **DoublonHolon4siteIdx;    /* DoublonHolon4siteIdx[idx][4*Nsite] */
...
// int NCisAjsCktAltCmuAnu, **CisAjsCktAltCmuAnuIdx; /* [NCisAjsCktAltCmuAnu][9]
// */ //Maxime
int NCisAjsLz, **CisAjsLzIdx,
    **iOneBodyGIdx; /* For Lanczos method only for rank 0*/
int NCisAjsCktAltLz, **CisAjsCktAltLzIdx;

/* Optimization flag */
int *OptFlag;       /* [NPara]  1: optimized, 0 or 2: fixed */
int AllComplexFlag; /* 0 -> all real variables, !=0-> including complex
                       variables*/
...
int NPara; /* the total number of variational prameters NPara= NProj + NSlater+
              NOptTrans */
...
int NOptTrans; /* the number of weights for OptTrans. This is used only for
...
double complex *SPGLCos,
    *SPGLSin; /* [NSPGaussLeg]  cos(beta/2) and sin(beta/2) */
...
double complex
    *SROptData; /* [2+NPara] storage for energy and variational parameters */
...
int NExcitation = 0, Excitation_W = 1,
    Excitation_L = 1; // Beware, NExcitation is the number of excitation per
                      // site, but now the file contains the excitations for
                      // each sites. NExcitation must be the same for each site
...
double complex *QCisAjsCktAltQ;
/* QCisAjsCktAltQ[NLSHam][NLSHam][NCisAjsCktAlt]*/ // TBC
double complex *LSLCisAjs; /* [NLSHam][NCisAjs]*/  // TBC

double *QCisAjsQ_real; /* QCisAjsQ[NLSHam][NLSHam][NCisAjs]*/ // TBC
double *QCisAjsCktAltQ_real;
/* QCisAjsCktAltQ[NLSHam][NLSHam][NCisAjsCktAlt]*/ // TBC
