...
//char *ReadBuffIntDim(FILE *fp, int *iNbuf, int *iExcitation_L, int *iExcitation_W, int *iNsite_b) {
char *ReadBuffIntDim(FILE *fp, int *iNbuf, int *iExcitation_L, int *iExcitation_W) {
  ...
  if (cerr != NULL) {
    ...
    // Get number of bath sites
    //printf("Reading number of bath sites\n");
    //cerr = fgets(ctmp2, sizeof(ctmp2) / sizeof(char), fp);
    //sscanf(ctmp2, "%s %d\n", ctmp, iNsite_b); //5
  }
  return cerr;
      ...
      if (fp == NULL) {
        ...
        switch (iKWidx) {
          ...
          case KWExcitation:   
            ...
            //cerr = ReadBuffIntDim(fp, &bufInt[IdxNExcitation], &bufInt[IdxExcitation_L], &bufInt[IdxExcitation_W], &bufInt[IdxNsite_b]);
...
//            break;
  ...
  PrintEnergy = bufInt[IdxPrintEnergy];
  PrintConfig = bufInt[IdxPrintConfig];
  PrintProgress = bufInt[IdxPrintProgress];  
  ExactSample = bufInt[IdxExactSample];
  ...
  TwoSz = bufInt[Idx2Sz];
  //Nsite_b = bufInt[IdxNsite_b]; //Number of bath sites
  //Nsite = Nsite - Nsite_b; //Number of cluster sites
  NExcitation = bufInt[IdxNExcitation]; 
  Excitation_L = bufInt[IdxExcitation_L];
  Excitation_W = bufInt[IdxExcitation_W];
  //Nsite = Excitation_L * Excitation_W;
  //printf("Nsite: %d, Nsite: %d\n", Nsite, Nsite);
  NExcitation_tot = NExcitation * Nsite;
  printf("Excitation_L: %d, Excitation_W: %d, NExcitation: %d, NExcitation_tot: %d\n", Excitation_L, Excitation_W, NExcitation, NExcitation_tot);
        ...
        case KWBF:
          if (NBackFlowIdx > 0) {
            ...
            while (fscanf(fp, "%d ", &i) != EOF) {
              if(fscanf(fp, "%d\n", &(OptFlag[fidx])) != 1) {
                fprintf(stderr, "fscanf: wrong number of parameters.\n");
                exit(-1);
              };
...
void SetDefaultValuesModPara(int *bufInt, double *bufDouble) {
  ...
  bufInt[IdxPrintEnergy] = 0;
  bufInt[IdxPrintConfig] = 0;
  bufInt[IdxPrintProgress] = 0;
  bufInt[IdxExactSample] = 0;
      ...
      switch (iKWidx) {
        case KWModPara:
          ...
          while (fgets(ctmp2, sizeof(ctmp2) / sizeof(char), fp) != NULL) {
            ...
            } else if (CheckWords(ctmp, "PrintEnergy") == 0) {
              bufInt[IdxPrintEnergy] = (int) dtmp;
	    } else if (CheckWords(ctmp, "PrintConfig") == 0) {
              bufInt[IdxPrintConfig] = (int) dtmp;
	    } else if (CheckWords(ctmp, "PrintProgress") == 0) {
              bufInt[IdxPrintProgress] = (int) dtmp;
	    } else if (CheckWords(ctmp, "ExactSample") == 0) {
              bufInt[IdxExactSample] = (int) dtmp;
