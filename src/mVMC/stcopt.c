...
#pragma loop noalias
  for (pi = 0; pi < 2 * nPara; pi++) {
    ...
    r[pi] = creal(srOptOO[(pi + 2) * (2 * srOptSize) + (pi + 2)]) -
            creal(srOptOO[pi + 2]) * creal(srOptOO[pi + 2]);
    // printf("DEBUG: pi=%d: %lf %lf
    // \n",pi,creal(srOptOO[pi]),cimag(srOptOO[pi]));
#ifdef _DEBUG_STCOPT
    fprintf(stderr, "DEBUG in %s (%d): r[%d] = %lf\n", __FILE__, __LINE__, pi,
            r[pi]);
#endif
  }
  ...
  si = 0;
  for (pi = 0; pi < 2 * nPara; pi++) {
    // printf("DEBUG: nPara=%d pi=%d OptFlag=%d
    ...
    if (sDiag < diagCutThreshold) { /* fixed by diagCut */
      ...
      smatToParaIdx[si] =
          pi; // si -> restricted parameters , pi -> full paramer 0 <-> 2*NPara
      si += 1;
    }
    // e
  }
  ...
  if (rank == 0) {
    if (info != 0)
      fprintf(stderr, "StcOpt: DPOSV info=%d\n", info);
    rmax = r[0];
    simax = 0;
    ;
    ...
    fprintf(FileSRinfo, "%5d %5d %5d %5d % .5e % .5e % .5e %5d\n", NPara, nSmat,
            optNum, cutNum, sDiagMax, sDiagMin, rmax, smatToParaIdx[simax]);
  }
