...
# 2x1 cluster
bath_sites = np.arange(nSites, nSites+Nb, 1)
cluster_site_coords = [[0, 0], [1, 0]]
bath_site_coords = [[-1, -1], [2, -1], [-1, 1], [2, 1]]
max_dist, nbb = 2, 2
...
def WriteExcitation():
    ...
    for r1 in range(nSites):
        ...
        # ra on cluster, rb on bath
        for dr1x in dr1_x:
            for dr1y in dr1_y:
                ra = find_neighbor_site(r1, dr1x, dr1y)
                # inds = np.where(np.abs(hop_mat[:,0]-r1)<1e-8)[0]
                for bsite in bath_sites:
                    # for ind in inds:
                    # if(hop_mat[ind,2]>nSites-1):
                    # rb = int(hop_mat[ind,2])
                    distx = np.abs(
                        cluster_site_coords[r1][0]-bath_site_coords[int(bsite)-nSites][0])
                    disty = np.abs(
                        cluster_site_coords[r1][1]-bath_site_coords[int(bsite)-nSites][1])
                    if (distx+disty <= max_dist):
                        rb = bsite
                        # condition to prevent redundant excitation
                        if ((r1, ra, rb) not in pairList_s4):
                            NN += 1
                            ...
                            pairList_s4.append((r1, ra, rb))
                            # condition to prevent redundant excitation
                            if ((r1, ra, rb) not in pairList_s3):
                                NN += 1
                                # everything matters for type 4
                                s3 += '%5d %4d %3d %3d \n' % (3, r1, ra, rb)
                                pairList_s3.append((r1, ra, rb))
        ...
        # BEWARE: number of excitation per lattice site must be the same
        # for every sites.
        if r1 == 0:
            NExcitation = NN
        else:
