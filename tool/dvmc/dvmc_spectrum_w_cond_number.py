...
def dvmc_spectrum(verbose=1) -> None:
    ...
    k_tol = 3.0
    ncut_AC = -1
    while (SV_AC_tmp[ncut_AC] < 1e-10):
        ncut_AC -= 1
    while (True):
        cond_num_AC = SV_AC_tmp[0]/SV_AC_tmp[ncut_AC]
        k_AC = math.log10(cond_num_AC)
        if (k_AC < k_tol):
            break
        ncut_AC -= 1

    ncut_CA = -1
    while (SV_CA_tmp[ncut_CA] < 1e-10):
        ncut_CA -= 1
    while (True):
        cond_num_CA = SV_CA_tmp[0]/SV_CA_tmp[ncut_CA]
        k_CA = math.log10(cond_num_CA)
        if (k_CA < k_tol):
            break
        ncut_CA -= 1

    nc_AC = len(SV_AC)+1+ncut_AC
    nc_CA = len(SV_CA)+1+ncut_CA
