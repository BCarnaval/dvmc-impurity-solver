...
int mergeOutputBin(int n_file, const char **string_list, int *n_exc, int Nsite,
                   int dummy, float *S_CA, float *S_AC, float *H_CA,
                   float *H_AC, int verbose) {
  ...
  for (ii = 0; ii < n_file; ii++) {
    ...
    if (NExcitation_tmp != NExcitation || Excitation_L_tmp != Excitation_L ||
        Excitation_W_tmp != Excitation_W) {
...
int greenFrom_e_U_Uinv_S(int sign, int Nw, double *w, int dim,
                         double complex *e, double complex *U,
                         double complex *Uinv_S, double Omega, double mu,
                         double eta, double complex *g) //)H, double *S,)
{
  int ii, nn;
...
int greenFrom_e_U_Uinv_S_general(int sign, int ll, int pp, int Ns, int Nw,
                                 double *w, int Nex, double complex *e,
                                 double complex *U, double complex *Uinv_S,
                                 double Omega, double mu, double eta,
                                 double complex *g) //)H, double *S,)
{
  int ii, nn, ll2, pp2;
  int ind1, ind2;

  for (ii = 0; ii < Ns * Ns * Nw; ii++)
    g[ii] = 0.0;
      ...
      for (ii = 0; ii < Ns; ii++) {
        for (jj = 0; jj < Ns; jj++) {
          double complex phase =
              cexp(I * (kx[mm] * x[ii] + ky[mm] * y[ii])) *
              cexp(-1.0 * I * (kx[nn] * x[jj] + ky[nn] * y[jj]));
