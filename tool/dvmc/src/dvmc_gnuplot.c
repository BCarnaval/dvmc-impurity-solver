...
void prepareTerminalInputs() {
  ...
  newSettings.c_lflag &=
      (~ICANON & ~ECHO); // turn off the ECHO and ICANON option
...
FILE *gpc_init_image() {
  ...
  pipe = popen("gnuplot > /dev/null 2>&1",
               "w"); // Open pipe to Gnuplot and check for error
  if (pipe == NULL) {
...
int gpc_plot_image(FILE *pipe, float *pData, int freq, int Nx, int Ny,
                   float zMin, float zMax, int Nw) {
  int i, j;
  char title[80];
  ...
  // fprintf (pipe, "set arrow 1 from %f,%f to %f,%f  lw 2 lc 'black'
  // front\n",chem_x,chem_y1,chem_x,chem_y2);

  fprintf(pipe,
          "set palette defined (0 'white',0.016 'white', 0.04 '#fff879',0.16 "
          "'#fe7e00',0.27 '#d81800', 0.4 '#780000', 0.6 'black')\n");
  fprintf(pipe,
          "set pm3d map\nset size square\nset cbtics ticspace\nset cbrange "
          "[%1.3f:%1.3f]\n",
          zMin, zMax);
...
int main(int argc, char *argv[]) {
  ...
  if (argc >= 5)
    freq = atoi(argv[4]);
  if (argc >= 6)
    cbar_max = atof(argv[5]);

  printf("w=%d    \n", freq);

  printf("Nx=%d,Ny=%d,Nw=%d\n", Nx, Ny, Nw);
  printf("First give focus to the command line and second press and hold z or "
         "x to change the frame\n");
      ...
      if (read(fileno(stdin), &ch, 2561) == 1) {
        ...
        if (c == 'x') {
          ...
          for (j = 0; j < (Ny + 1); j++) // For every row
          {
            ...
            {
              fprintf(tmpSlice, "%f ",
                      dataCube[freq * Nx * Ny + (((i + Nx / 2) % Nx) * Ny) +
                               ((j + Ny / 2) % Ny)]);
            }
            fprintf(tmpSlice, "\n");
          }
